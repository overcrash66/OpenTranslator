...
class CustomTranslator:
	...
	def process_audio_chunk(self, input_path, target_language, chunk_idx, output_path,source_se,target_se,tone_color_converter):
		try:
			self.load_model()
			...
			if target_language != "en":
				...
				self.generate_audio(translated_text, Translation_chunk_output_path, target_language, target_se,input_path,source_se,tone_color_converter)
				
				logging.info(f"Processing successful. Translated text: {translated_text}")
				return translated_text
			else:
				self.generate_audio(transcription, Translation_chunk_output_path, target_language, target_se,input_path,source_se,tone_color_converter)
	...
	def generate_audio(self, text, output_path, target_language,target_se, input_path,source_se,tone_color_converter):
		tts = gTTS(text, lang=target_language, slow=False)
		tts.save(output_path+'-test')

		# Run the tone color converter
		encode_message = "@MyShell"
		tone_color_converter.convert(
			audio_src_path=output_path+'-test', 
			src_se=source_se, 
			tgt_se=target_se, 
